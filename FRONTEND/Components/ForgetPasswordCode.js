...
import checkForgetPasswordCodeApi from "../api/checkForgetPasswordCodeApi";
...
export default class ForgetPasswordCode extends Component {
  ...
  goResetPassword(email){
    const { navigator } = this.props;
    navigator.push({ name: 'RESET_PASSWORD',email}); 
  }
  ...
  _onFulfill(code){
    ...
    checkForgetPasswordCodeApi(this.props.email, code)
    .then((responseJson)=>{
      this.toggleModal();
      ...
      if(responseJson.result==="success"){
        Alert.alert(
          ...
          [{text: 'OK',
          onPress: () => this.goResetPassword(this.props.email)
        }],
          { cancelable: false }
        );
      }

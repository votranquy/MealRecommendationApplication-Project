...
import getTopFoodViewApi from "../../api/getTopFoodViewApi";     //Nolocation
import getTopFoodViewApi2 from "../../api/getTopFoodViewApi2"; //Location
...
export default class TopFoodView extends Component {
  ...
  componentDidMount(){
    ...
    .then(region => {
      ...
      if(region===""){return getTopFoodViewApi(this.state.page)}
      else{  return getTopFoodViewApi2(this.state.page, this.state.region.latitude, this.state.region.longitude) }
     })
    .then((responseJson)=>{
    ...
    if(property.food_name == "");
      ...
      <TouchableOpacity 
        ...
        <View style={styles.ctnInfomation}>
          ...
          <View style={styles.cntText}>
            <Text style={styles.txtRate}>{String(property.total_view)} lượt xem</Text>
          </View>
          <View style={styles.cntText}>

...
import getTopVoteApi from "../../api/getTopVoteApi";     //Nolocation
import getTopVoteApi2 from "../../api/getTopVoteApi2"; //Location
import getLocation from "../../api/getLocation";

export default class TopVote extends Component {
  ...
  componentDidMount(){
    ...
    .then(region => {
      ...
      if(region===""){return getTopVoteApi(this.state.page)}
      else{  return getTopVoteApi2(this.state.page, this.state.region.latitude, this.state.region.longitude) }
     })
    .then((responseJson)=>{
    ...
    .catch(error=>{
      console.log('GETTOPVOTE_ERROR',error);
      this.setState({
        isLoading: false,
        dataSource: this.state.dataSource.cloneWithRows([]),
      });
    ...
    if(property.food_name == "");
      ...
      <TouchableOpacity 
        ...
        <View style={styles.ctnInfomation}>
          ...
          <View style={styles.cntText}>
            <Text style={styles.txtRate}>{String(property.total_vote)} lượt đánh giá</Text>
          </View>
          <View style={styles.cntText}>
  ...
  loadMore(){
    ...
    .catch(error=>{
      console.log('GETMORETOPVIEW_ERROR',error);
      this.setState({
        isLoadingMore: false,
        dataSource: this.state.dataSource.cloneWithRows([]),
      });
